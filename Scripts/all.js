//JAVASCRIPT APPLIED TO ALL PAGES
//THIS SHOULD CONTAIN ANY ELEMENTS/COMPONENTS USED ACROSS MULTIPLE PAGES

// IIFE - Immediately Invoked Function Expression
(function($, window, document) {
  // The $ is now locally scoped

  // Listen for the jQuery ready event on the document
  $(function() {
    // The DOM is ready!
    backgroundSlick();
    // particleBackground();
    aboutPopup();
    listenPopup();
  });

  function backgroundSlick() {
    $(".background .slides").slick({
      fade: true,
      arrows: false,
      dots: false,
      infinite: true,
      speed: 5000,
      autoplay: true,
      autoplaySpeed: 8000
    });
  }

  function aboutPopup() {
    $(".e-aboutlink").on("click", function(e) {
      e.preventDefault();
      $("body").addClass("about-open");
    });

    $(".m-about .e-close").on("click", function() {
      $("body").removeClass("about-open");
    });
  }

  function listenPopup() {
    $(".e-listenlink").on("click", function(e) {
      e.preventDefault();
      $("body").addClass("listen-open");
    });

    $(".m-listen .e-close").on("click", function() {
      $("body").removeClass("listen-open");
    });
  }

  function particleBackground() {
    particlesJS("particleCanvas", {
      particles: {
        number: {
          value: 80,
          density: {
            enable: true,
            value_area: 800
          }
        },
        color: {
          value: "#333333"
        },
        shape: {
          type: "circle",
          stroke: {
            width: 0,
            color: "#000000"
          },
          polygon: {
            nb_sides: 5
          },
          image: {
            src: "img/github.svg",
            width: 100,
            height: 100
          }
        },
        opacity: {
          value: 0.5,
          random: false,
          anim: {
            enable: false,
            speed: 1,
            opacity_min: 0.1,
            sync: false
          }
        },
        size: {
          value: 3,
          random: true,
          anim: {
            enable: false,
            speed: 40,
            size_min: 0.1,
            sync: false
          }
        },
        line_linked: {
          enable: true,
          distance: 150,
          color: "#444444",
          opacity: 0.4,
          width: 1
        },
        move: {
          enable: true,
          speed: 1,
          direction: "none",
          random: false,
          straight: false,
          out_mode: "out",
          bounce: false,
          attract: {
            enable: false,
            rotateX: 600,
            rotateY: 1200
          }
        }
      },
      interactivity: {
        detect_on: "canvas",
        events: {
          onhover: {
            enable: false,
            mode: "repulse"
          },
          onclick: {
            enable: true,
            mode: "push"
          },
          resize: true
        },
        modes: {
          grab: {
            distance: 400,
            line_linked: {
              opacity: 1
            }
          },
          bubble: {
            distance: 400,
            size: 40,
            duration: 2,
            opacity: 8,
            speed: 3
          },
          repulse: {
            distance: 200,
            duration: 0.4
          },
          push: {
            particles_nb: 4
          },
          remove: {
            particles_nb: 2
          }
        }
      },
      retina_detect: true
    });
  }

  // The rest of code goes here!
})(window.jQuery, window, document);
